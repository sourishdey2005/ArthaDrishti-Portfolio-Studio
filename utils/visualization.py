...
class Visualizer:
    ...
    def plot_monthly_returns_heatmap(self, returns_df, asset=None):
        ...
        monthly = returns_df[asset].resample('ME').apply(lambda x: (1 + x).prod() - 1)
    ...
    def plot_calendar_return_bars(self, returns_df, asset=None):
        asset = asset or returns_df.columns[0]
        monthly = returns_df[asset].resample('ME').apply(lambda x: (1 + x).prod() - 1) * 100
    ...
    def plot_monthly_boxplot(self, returns_df):
        monthly = returns_df.resample('ME').apply(lambda values: (1 + values).prod() - 1) * 100
        fig = go.Figure()
